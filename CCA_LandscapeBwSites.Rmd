...
CCA_site_all2 <- ggplot() + coord_equal() + 
    ...
    geom_text(data = all_cca_vec_sig,
          aes(x = c(0.40+.40, 0.71+.62, 0.76+.40, 0.19-.32),
...
dist_site_ls %>% filter(col != 'WHT-PASVT') %>% filter(row != 'WHT-PASVT') -> dist_site_ls
dim(test) #105 comps

#recent climate
dist_site_ls$pr90X <- SbyE_site$Precip_90day[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$pr90Y <- SbyE_site$Precip_90day[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$pr90 <- abs(dist_site_ls$pr90X - dist_site_ls$pr90Y)

dist_site_ls$lt90X <- SbyE_site$LT_90day[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$lt90Y <- SbyE_site$LT_90day[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$lt90 <- abs(dist_site_ls$lt90X - dist_site_ls$lt90Y)

dist_site_ls$ht90X <- SbyE_site$HT_90day[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$ht90Y <- SbyE_site$HT_90day[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$ht90 <- abs(dist_site_ls$ht90X - dist_site_ls$ht90Y)

#historic climate
dist_site_ls$mapX <- SbyE_site$MAP[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$mapY <- SbyE_site$MAP[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$map <- abs(dist_site_ls$mapX - dist_site_ls$mapY)

dist_site_ls$mltX <- SbyE_site$MLT[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$mltY <- SbyE_site$MLT[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$mlt <- abs(dist_site_ls$mltX - dist_site_ls$mltY)

dist_site_ls$mhtX <- SbyE_site$MHT[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$mhtY <- SbyE_site$MHT[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$mht <- abs(dist_site_ls$mhtX - dist_site_ls$mhtY)
...
# soil
dist_site_ls$moistX <- SbyE_site$soilmoist[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$moistY <- SbyE_site$soilmoist[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$moist <- abs(dist_site_ls$moistX - dist_site_ls$moistY)

dist_site_ls$hmX <- SbyE_site$HM_perc[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$hmY <- SbyE_site$HM_perc[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$hm <- abs(dist_site_ls$hmX - dist_site_ls$hmY)

dist_site_ls$phX <- SbyE_site$pH[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$phY <- SbyE_site$pH[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$ph <- abs(dist_site_ls$phX - dist_site_ls$phY)

# plant
dist_site_ls$laiX <- SbyE_site$LAI.mean[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$laiY <- SbyE_site$LAI.mean[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$lai <- abs(dist_site_ls$laiX - dist_site_ls$laiY)

dist_site_ls$hgtX <- SbyE_site$plantHt[match(dist_site_ls$col, SbyE_site$nestSite)]
dist_site_ls$hgtY <- SbyE_site$plantHt[match(dist_site_ls$row, SbyE_site$nestSite)]
dist_site_ls$hgt <- abs(dist_site_ls$hgtX - dist_site_ls$hgtY)

dist_site_ls %>% dplyr::select(-ends_with("X"), -ends_with("Y")) -> dist_site_ls
```

### 3b) OLS models
```{r}
# best subsets approach
BW1 <- lm(value ~ pr90 + lt90 + ht90 + map + mlt + mht +
              crop10 + veg10 + crop1 + veg1 +
              moist + hm + ph + lai + hgt, data=dist_site_ls)
...
```{r}
BW2 <- lm(value ~ pr90 + lt90 + map + mlt + mht + #ht90 +
              crop10 + veg10 + crop1 + veg1 +
              moist + hm + ph + lai + hgt, data=dist_site_ls)
olsrr::ols_coll_diag(BW2)

BW3 <- lm(value ~ pr90 + lt90 + map + mlt + mht + #ht90 + crop10 + 
              veg10 + crop1 + veg1 +
              moist + hm + ph + lai + hgt, data=dist_site_ls)
olsrr::ols_coll_diag(BW3)

BW4 <- lm(value ~ pr90 + lt90 + map + mlt + mht + #ht90 + crop10 + veg1 +
              veg10 + crop1 + 
              moist + hm + ph + lai + hgt, data=dist_site_ls)
olsrr::ols_coll_diag(BW4)

BW5 <- lm(value ~ pr90 + lt90 + map + mlt + mht + #ht90 + crop10 + veg1 + crop1 +
              veg10 +  
              moist + hm + ph + lai + hgt, data=dist_site_ls)
olsrr::ols_coll_diag(BW5)

BW6 <- lm(value ~ pr90 + lt90 + map +  mht + #ht90 + crop10 + veg1 + crop1 + mlt +
              veg10 +  
              moist + hm + ph + lai + hgt, data=dist_site_ls)
olsrr::ols_coll_diag(BW6)
```

```{r}
BW7 <- lm(value ~ lt90 + map +  mht + #ht90 + crop10 + veg1 + crop1 + mlt + pr90 + 
              veg10 +  
              moist + hm + ph + lai + hgt, data=dist_site_ls)
olsrr::ols_coll_diag(BW7)
```

```{r}
olsrr::ols_step_best_subset(BW7)
