...
```{r, fig.width = 7}
par(mfrow = c(1,3))
hist(log(SbyE_new$prop_Pathotroph))
hist(log(SbyE_new$prop_Symbiotroph))
...
sapr_bySite <- ggplot(SbyE_new, aes(y = prop_Saprotroph, x = nestSite, fill = factor(nestSite))) +
    ...
    geom_jitter(width = .1)+
    scale_x_discrete("Site") +
    scale_y_continuous("Proportion of Saprobe Sequences", limits = c(-0.01,0.65)) + 
    guides(fill = 'none') +
    theme(axis.text.x = element_text(angle = 90))
...
test <- lm(prop_Saprotroph ~ LT_90day, data = SbyE_new)
Anova(test, type = "III")
summary(test)$coefficients
ggplot(SbyE_new, aes(x=LT_90day, y = prop_Saprotroph, colour = nestSite)) +
