...
```
```{r, fig.width = 9}
ggplot(dat, aes(x = crop, y = Abundance, fill = nestSite)) +
...
```{r}
dim(dat)  #59019=191*309 
dim(FG_sub)
dat$trophicMode <- factor(dat$trophicMode)
dat$myTrophicMode <- dat$trophicMode
levels(dat$myTrophicMode) <- list(
...
#i think something like this will work, but I need to go back and use all 595 asvs
```
