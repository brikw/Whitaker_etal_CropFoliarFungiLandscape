...
# NOTE - MASSIVELY MANUALLY EDITED, converted to excel with color coding, re-saved as MBLand_taxa.taxa.txt to match funguild naming convention
```
...
```{r}
# head(FG_results)
# levels_FG <- levels(FG_results$OTU)
# ps_land

# table(FG_results$confidenceRanking) # 77 Possibe
# table(FG_results$trophicMode)       # 202 na's
...
levels(FG_results$myTrophicMode) <- list(
    "Patho-other" = c("Pathotroph", "Pathotroph-Saprotroph"),
    "Symbio-other" = c("Symbiotroph", "Saprotroph-Symbiotroph"),
...
#subset funguild output for use as a tax_table()
FG_results %>% select(OTU, Kingdom, Phylum, Order, Class, Family, Genus, Species, taxon, trophicMode, confidenceRanking, myTrophicMode) -> FG_sub_all
...
# create a new phyloseq obj with FUNGuild output as the taxa table
merge_phyloseq(otu_table(ps_land), sample_data(ps_land), refseq(ps_land), tax_table(as.matrix(FG_sub_all))) -> ps_FG_all
