...
pcoa_site <- ggplot() + coord_equal() + 
    ...
    annotate(geom = "text", x = c(-24.5+10, -6.6+10, 35.6-10, -5.1+10), 
                            y = c(-25.4, 9.0, -11.6, 27.4), 
             label = levels(pcoa.dat$species), colour = spp_color) +
    annotate(geom = "text", x = -30, y = 35, label = "A)", size = 5)
pcoa_site

#tiff("./figures/PCoA_site.tiff", width=6, height=6, units="in", res=600)
pcoa_site
#dev.off()
```
...
swch.gxe <- ggplot() + #coord_equal() + 
    ...
    scale_y_continuous(paste("PCoA 2 (", swchvar2, "%)", sep = "")) +
    ...
    annotate(geom = "text", x = -39, y = 38, label = "E)", size = 5)
...
crn.gxe <- ggplot() + #coord_equal() + 
    ...
    geom_point(data = crn.dat, aes(x = V1, y = V2, 
          shape = factor(OVT.Variety), color = site) , size = 2) +
    #guides(shape = 'none', color = 'none') +
    scale_shape_manual("", values = rep(c(17, 16, 15), 4) ) +
    scale_colour_manual(values = rep(crn_color, each=4) ) +
    scale_linetype_manual("", values = rep(c(1,2,3),4) ) +
...
wht.gxe <- ggplot() + #coord_equal() + 
    ...
    annotate(geom = "text", x = -30, y = 20, label = "D)", size = 5)
...
soy.gxe <- ggplot() + #coord_equal() + 
    ...
    annotate(geom = "text", x = -21, y = 19.5, label = "C)", size = 5)
...
#tiff("./figures/GxE_by_spp.tiff", width=9, height=8, units="in", res=600)
multiplot(crn.gxe, soy.gxe, wht.gxe, swch.gxe, cols = 2)
#dev.off()
```
