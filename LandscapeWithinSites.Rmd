...
land_A <- ggplot(RichLU_long[RichLU_long$landcover.type == "Perc_veg_1km",],
  ...
  geom_point(size = 2) +
  geom_errorbar(aes(ymin=Rich_nosing-Rich_nosing_ci, ymax=Rich_nosing+Rich_nosing_ci), width=.2) +
  ...
  annotate(geom = "text", x = 3, y = 84.65, label = "A)")
...
land_B <- ggplot(RichLU_long[RichLU_long$landcover.type == "Perc_veg_1km",],
  ...
  scale_color_manual("", values = site_color) +
  theme(legend.position="none") +
  annotate(geom = "text", x = 3, y = 61, label = "B)")


#tiff("./figures/Dissim & Richness (Alt) ~ Perc_Veg.tiff", width=7.5, height=4, units="in", res=600)
