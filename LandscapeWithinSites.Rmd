...
```{r}
colnames(RichLU)
RichLU %>% filter(site != 'WHT-PASVT') -> RichLU
...
model1 <- lm(Rich_nosing ~  #Lat.N + Lon.W + Elev +  MLT + MHT + 
                 ...
                 soilmoist + LAI.mean + plantHt + HM_perc + pH, data=RichLU)
Anova(model1)
...
model3 <- lm(Rich_nosing ~  #HT_90day + HM_perc + 
                 Precip_90day + LT_90day +  MAP + 
                 Perc_crop_10km + Perc_veg_10km + Perc_crop_1km + Perc_veg_1km +
                 soilmoist + LAI.mean + plantHt + pH, data=RichLU)
...
model7 <- lm(Rich_nosing ~  #HT_90day + HM_perc + Perc_veg_10km + Perc_crop_10km +
                 ...
                 Precip_90day + LT_90day +  MAP + 
                 Perc_veg_1km +
                 soilmoist + LAI.mean + pH, data=RichLU)
...
rich_lt <- ggplot(RichLU_long[RichLU_long$sig_vars == "LT_90day",],
  ...
  theme(legend.position="none", 
        axis.title = element_text(size = 28), axis.text = element_text(size = 28)) +
  annotate(geom = "text", x = -5, y = 84.65, label = "B)", size = 9)
...
bdiv_veg1km <- ggplot(RichLU_long[RichLU_long$sig_vars == "Perc_veg_1km",],
  ...
  theme(legend.position="none", 
        axis.title = element_text(size = 28), axis.text = element_text(size =28)) +
  annotate(geom = "text", x = 3, y = 61, label = "C)", size = 9)
...
bdiv_lt <- ggplot(RichLU_long[RichLU_long$sig_vars == "LT_90day",],
  ...
  theme(legend.position="none", 
        axis.title = element_text(size = 28), axis.text = element_text(size =28)) +
  annotate(geom = "text", x = -5, y = 61, label = "D)", size = 9)


#tiff("./figures/Dissim & Richness (Alt) ~ Perc_Veg.tiff", width=13.2, height=13.2, units="in", res=600)
