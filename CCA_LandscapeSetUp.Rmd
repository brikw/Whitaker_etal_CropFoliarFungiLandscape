...
CCA_site_all_16 <- ggplot() + coord_equal() + 
    ...
    geom_text(data = all_cca_vec_sig_16,
          aes(x = c(0.89+0.1, 0.71+0.1),
              y = c(0.39+.25,    -0.52-.25), label = vecs),
                 size = 5, colour = "gray30", fontface = 'bold') +
    scale_colour_manual("", values = site_color) +
    scale_x_continuous(paste("CCA Axis-1 (", explain_all_16[1], "%)", sep = "")) +
    scale_y_continuous(paste("CCA Axis-2 (", explain_all_16[2], "%)", sep = "")) +
    guides(colour=guide_legend(ncol=2)) + #guides(colour = 'none') + 
    theme(panel.grid.major=element_line(colour = 'gray90', size = 0.5), 
          ...
          legend.background = element_rect(colour = NA, fill = NA),
          legend.position = "right", legend.margin=margin(0,2,0,0), 
          legend.box.margin=margin(-10,-6,-6,-10)) #t,r,b,l

#tiff("./figures/CCA_all_SiteLevel_16sites.tiff", width=180, height=80, units="mm", res=600)
CCA_site_all_16
#dev.off()
```
